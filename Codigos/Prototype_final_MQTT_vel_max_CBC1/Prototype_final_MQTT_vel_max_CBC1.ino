...
//Configuracion APN:
  const char* APN = "movistar.pe";//chip.entel.pe para entel y claro.pe para claro
...
// Configuración del broker MQTT
  ...
  const char* client_id = "CBC_1";  // Client ID
  const char* topic1 = "cbc/gps/datareceived/CBC_1";   //Topico donde se enviarán los datos
  ...
  const char* topic4 = "cbc/gps/historydata/CBC_1";    //topico donde se recibiran datos historicos
  const char* topic5 = "cbc/gps/deleteSD";       //topico donde se enviara un comando para borrar la SD
  const char* topic6 = "cbc/gps/stateESP32/CBC_1";     //topico para mandar mensajes del estado del ESP32
  const char* topiclwt = "cbc/gps/off/CBC_1";   //topico para last will
  const char* lwt = "CBC_1 off";         //Last will message
...
void loop2(void *parameter) {
  ...
  for (;;) {
    ...
    /*if (!mqtt_connected) {
      ...
      if (!mqtt_connected) {
        vTaskDelay(pdMS_TO_TICKS(5000)); // Esperar antes de volver a intentar
